...
router.get('/data/:tableName', async (req, res) => {
    try {
        ...
        const pool = await (0, connection_1.connectDB)(database);
        ...
        const result = await pool.request()
            .input('limit', mssql_1.default.Int, parseInt(limit))
            .input('offset', mssql_1.default.Int, parseInt(offset))
            .query(`
        SELECT *
        FROM ${tableReference}
        ORDER BY 1
    ...
    `;
        const columnsResult = await pool.request()
            .input('tableName', mssql_1.default.VarChar, tableName)
            .query(columnsQuery);
...
// Delete database
router.delete('/databases/:databaseName', async (req, res) => {
    try {
...
router.get('/databases/:databaseName/tables', async (req, res) => {
    try {
        const { databaseName } = req.params;
        ...
        const pool = await (0, connection_1.connectDB)(databaseName);
...
router.get('/tables/:tableName/triggers', async (req, res) => {
    try {
        const { tableName } = req.params;
    ...
    `;
        const schemaRes = await pool.request().input('tableName', mssql_1.default.VarChar, tableName).query(schemaQuery);
    ...
    `;
        const result = await pool.request()
            .input('tableName', mssql_1.default.VarChar, tableName)
            .input('schemaName', mssql_1.default.VarChar, schemaName)
            .query(query);
...
router.post('/tables/:tableName/triggers', async (req, res) => {
    try {
        const { tableName } = req.params;
    ...
    `;
        const schemaRes = await pool.request().input('tableName', mssql_1.default.VarChar, tableName).query(schemaQuery);
        const schemaName = schemaRes.recordset[0]?.table_schema || 'dbo';
        const eventsClause = events.map((e) => e.toUpperCase()).join(', ');
...
`;
        ...
        request.input('sqlText', mssql_1.default.NVarChar(mssql_1.default.MAX), sqlText);
...
router.put('/tables/:tableName/triggers/:triggerName', async (req, res) => {
    try {
...
// Delete trigger
router.delete('/tables/:tableName/triggers/:triggerName', async (req, res) => {
    try {
        const { tableName, triggerName } = req.params;
...
router.get('/procedures', async (req, res) => {
    ...
    `;
        ...
        const request = pool.request();
        if (schema)
            request.input('schemaName', mssql_1.default.VarChar, schema);
        if (search)
            request.input('searchLike', mssql_1.default.VarChar, `%${search}%`);
        let result;
        try {
      ...
      `;
            if (schema)
                fallback += ` AND ROUTINE_SCHEMA = @schemaName`;
            if (search)
                fallback += ` AND ROUTINE_NAME LIKE @searchLike`;
...
router.get('/procedures/:procedureName', async (req, res) => {
    try {
        ...
        const pool = await (0, connection_1.connectDB)(database);
        const request = pool.request();
        request.input('procName', mssql_1.default.VarChar, procedureName);
        if (schema)
            request.input('schemaName', mssql_1.default.VarChar, schema);
        const query = `
...
router.post('/procedures', async (req, res) => {
    try {
        const { database, schema = 'dbo', procedureName, definition } = req.body;
        if (!procedureName || !/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(procedureName)) {
            return res.status(400).json({ error: 'Invalid procedure name' });
        }
        if (database && !/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(database)) {
...
router.put('/procedures/:procedureName', async (req, res) => {
    try {
        const { procedureName } = req.params;
        const { database, schema = 'dbo', definition } = req.body;
        if (!procedureName || !/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(procedureName)) {
            return res.status(400).json({ error: 'Invalid procedure name' });
        }
        if (database && !/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(database)) {
...
router.delete('/procedures/:procedureName', async (req, res) => {
    try {
        const { procedureName } = req.params;
        const { database, schema = 'dbo' } = req.body;
        if (!procedureName || !/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(procedureName)) {
            return res.status(400).json({ error: 'Invalid procedure name' });
        }
        if (database && !/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(database)) {
...
// Add column to table
router.post('/tables/:tableName/columns', async (req, res) => {
    try {
        const { tableName } = req.params;
...
router.post('/backup', async (req, res) => {
    try {
        ...
        const pool = await (0, connection_1.connectDB)();
        const request = pool.request();
        request.input('dbName', mssql_1.default.VarChar, database);
        request.input('path', mssql_1.default.NVarChar(mssql_1.default.MAX), backupPath);
...
router.post('/restore', async (req, res) => {
    try {
        const { database, backupPath, withReplace = true } = req.body;
        if (!database || !/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(database)) {
            return res.status(400).json({ error: 'Invalid database name' });
        }
        if (!backupPath || typeof backupPath !== 'string' || backupPath.length < 4) {
